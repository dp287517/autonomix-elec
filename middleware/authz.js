...
// middleware/authz.js — robust auth middleware under middleware/
...
async function requireAuth(req, res, next){
  try{
    ...
    req.user.id = req.user.uid; // back-compat
    ...
    if (req.query && req.query.account_id){
      ...
      const r = await pool.query(`SELECT role FROM public.user_accounts WHERE user_id=$1 AND account_id=$2 LIMIT 1`, [req.user.uid, accountId]);
    ...
    if (!role){
      const r = await pool.query(`SELECT account_id, role FROM public.user_accounts WHERE user_id=$1 ORDER BY account_id ASC LIMIT 1`, [req.user.uid]);
