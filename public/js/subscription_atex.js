...
(()=>{
  ...
  async function choose(t){
    ...
    let r;
    try{
      r = await fetch(`${API}/subscriptions/${APP}/choose?account_id=${aid}`, { method:'POST', headers: headers(), body: JSON.stringify({ tier: t }) });
  ...
  document.addEventListener('DOMContentLoaded', async ()=>{
    ...
    document.querySelector('#btn-tier-free')?.addEventListener('click', ()=>choose(1));
    document.querySelector('#btn-tier-personal')?.addEventListener('click', ()=>choose(2));
    document.querySelector('#btn-tier-pro')?.addEventListener('click', ()=>choose(3));
  });
})();