...
(function(){
  ...
  function getAccountId(){
    try{
      ...
      const id = fromQS || stored || localStorage.getItem('selected_account_id') || '10';
      ...
      <div class="modal-dialog modal-lg modal-dialog-scrollable"><div class="modal-content">
        ...
        <div class="modal-body">
          ${atts.length ? atts.map((a,i)=>{
            const url = a && (a.url||a.href||a.path||a.data||'') || '';
    ...
    const html = `
      <div class="modal fade" id="${id}" tabindex="-1">
        <div class="modal-dialog modal-lg"><div class="modal-content">
          ...
          <div class="modal-body"><pre style="white-space:pre-wrap; word-break:break-word; background:#f8f9fa; padding:12px; border-radius:8px; border:1px solid #e9ecef; margin:0;">${text}</pre></div>
          <div class="modal-footer"><button class="btn btn-secondary" data-bs-dismiss="modal">Fermer</button></div>
        </div></div>
      </div>`;
    document.body.insertAdjacentHTML('beforeend', html);
  ...
  async function handleOpenImportColumns(e){
    e?.preventDefault();
    try{
      const r = await fetch(API.importColumns, { cache:'no-store' });
      if(!r.ok) throw new Error('Erreur API');
  ...
  async function handleImportExcel(){
    try{
      ...
      if(!f){ toast('Choisis un fichier .csv ou .xlsx','warning'); return; }
      const fd = new FormData(); fd.append('file', f, f.name);
      const r = await fetch(API.importExcel, { method:'POST', body: fd });
      if(!r.ok) throw new Error('Erreur import');
      const res = await r.json();
      toast(`Import OK — ajoutés: ${res.inserted||0}, mis à jour: ${res.updated||0}`,'success');
      await loadEquipments();
    }catch(err){ toast('Erreur import: '+(err.message||err),'danger'); }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnImport')?.addEventListener('click', handleImportExcel);
