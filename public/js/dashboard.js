// /public/js/subscription_atex.js (no-login build)
(function () {
  ...
  async function getCurrentTier(accountId) {
    const r = await fetch(`${API}/subscriptions/ATEX?account_id=${accountId}`, { cache: 'no-store' });
    if (r.status === 403) return { tier: 1, source: 'forbidden' };
    if (!r.ok) throw new Error('subscriptions ' + r.status);
    return r.json();
  }
  async function getOwners(accountId) {
    const r = await fetch(`${API}/accounts/${accountId}/owners`);
    if (!r.ok) return [];
    const d = await r.json().catch(() => ({}));
    return Array.isArray(d.owners) ? d.owners : [];
  }
  async function getMembers(accountId) {
    const r = await fetch(`${API}/accounts/members/ATEX?account_id=${accountId}`);
    if (!r.ok) return { members: [], seats_assigned: 0 };
    const d = await r.json().catch(() => ({}));
    return { members: Array.isArray(d.members) ? d.members : [], seats_assigned: Number(d.seats_assigned ?? d.members?.length ?? 0) };
  }
  async function choosePlan(accountId, tierOrPlan) {
    const body = typeof tierOrPlan === 'number' ? { tier: tierOrPlan } : { plan: String(tierOrPlan).toLowerCase() };
  ...
  // ---- Renders identiques à l’original ----
  function setPlanButtonsState(currentTier, myRole) {
    ...
    if (myRole === 'member') [btnFree, btnPer, btnPro].forEach(b => { if (b) b.title = 'Seul un owner/admin peut modifier l’abonnement.'; });
  }
