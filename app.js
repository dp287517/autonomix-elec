...
// (Optionnel) EPD store si présent
let epdStore;
try {
  epdStore = pickRouter(require('./routes/epdStore'), 'routes/epdStore');
} catch {
  console.warn('[Boot] routes/epdStore absent (OK).');
}
...
// (Facultatif) alias convivial vers la page publique
app.get(['/atex', '/atex-control'], (_req, res) => {
