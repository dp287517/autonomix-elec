...
router.post('/accounts/invite', requireAuth, requireRole('owner','admin'), async (req, res) => {
  try{
    ...
    const { email, role = 'member', appCode = 'ATEX' } = req.body || {};
    ...
    if (u.rowCount) {
      ...
      const ins = await pool.query(
        `INSERT INTO public.users(email, name, password) VALUES ($1,$2,'') RETURNING id`,
    ...
    res.status(201).json({ ok: true, invited: email, role, app: appCode, seats_total: seats });
  }catch(e){
...
router.get('/accounts/members/:appCode', requireAuth, requireRole('owner','admin'), async (req, res) => {
  try{
    ...
    `, [req.account_id, appCode]);
    res.json({ app: appCode, members: r.rows });
  }catch(e){
