...
router.get('/equipements', async (req, res) => {
  ...
  try {
    ...
    const allDisjoncteurs = disjoncteursResult.rows.flatMap(row =>
      Array.isArray(row.disjoncteurs)
        ? row.disjoncteurs.map(d => ({ ...d, equipmentType: 'disjoncteur' }))
        : []
    );
    const uniqueDisjoncteurs = Array.from(
      new Map(allDisjoncteurs.map(d => [`${d.equipmentType}-${d.marque}-${d.ref || d.id}`, d])).values()
    );
...
router.get('/tableaux/:id', async (req, res) => {
  ...
  try {
    ...
    const tableau = {
      id: t.rows[0].id,
      disjonctreurs: t.rows[0].disjoncteurs || [],
      disjoncteurs: t.rows[0].disjoncteurs || [],
      autresEquipements: e.rows.map(r => ({
        id: r.equipment_id,
...
router.get('/tableaux', async (req, res) => {
  ...
  try {
    ...
    const tableaux = t.rows.map(row => ({
      ...
      autresEquipements: e.rows
        .filter(r => r.tableau_id === row.id)
        .map(r => ({ id: r.equipment_id, equipmentType: r.equipment_type, ...r.data })),
...
router.post('/tableaux', async (req, res) => {
  ...
  try {
    ...
    if (!Array.isArray(disjoncteurs) || !Array.isArray(autresEquipements))
      throw new Error('disjoncteurs et autresEquipements doivent être des tableaux');
    ...
    for (const d of disjoncteurs) {
      if (d.linkedTableauIds && d.linkedTableauIds.length) {
        const invalidIds = d.linkedTableauIds.filter(
          lid => lid === id || !/^[\p{L}0-9\s\-_:]+$/u.test(lid)
        );
        if (invalidIds.length)
          throw new Error(
            `IDs de tableaux liés invalides pour disjoncteur ${d.id}: ${invalidIds.join(', ')}`
          );
        const linked = await client.query('SELECT id FROM tableaux WHERE id = ANY($1)', [
          d.linkedTableauIds
        ]);
        if (linked.rows.length !== d.linkedTableauIds.length) {
          const missing = d.linkedTableauIds.filter(
            lid => !linked.rows.some(r => r.id === lid)
          );
          throw new Error(
            `Tableaux liés non trouvés pour disjoncteur ${d.id}: ${missing.join(', ')}`
          );
        }
      }
      const errs = validateDisjoncteurData(d);
      if (errs.length)
        throw new Error(`Données invalides pour disjoncteur ${d.id}: ${errs.join('; ')}`);
    }
    for (const e of autresEquipements) {
      const errs = validateEquipementData(e);
      if (errs.length)
        throw new Error(`Données invalides pour équipement ${e.id}: ${errs.join('; ')}`);
    }
    if (isHTA) {
    ...
    const normalizedDisjoncteurs = disjoncteurs.map(d => {
      ...
      return {
        ...
        cableLength: isNaN(parseFloat(d.cableLength))
          ? d.isPrincipal
            ? 0
            : 20
        ...
        charge: d.charge || 80,
        linkedTableauIds: Array.isArray(d.linkedTableauIds)
          ? d.linkedTableauIds
          : d.linkedTableauId
          ? [d.linkedTableauId]
          : [],
...
router.put('/tableaux/:id', async (req, res) => {
  ...
  try {
    ...
    for (const d of disjoncteurs) {
      ...
      if (errs.length)
        throw new Error(
          `Données invalides pour disjoncteur ${d.id || 'sans ID'}: ${errs.join('; ')}`
        );
    }
    for (const e of autresEquipements) {
...
router.delete('/tableaux/:id', async (req, res) => {
  ...
  try {
    ...
    const linkedBy = linkedCheck.rows.filter(row =>
      (row.disjoncteurs || []).some(
        d => Array.isArray(d.linkedTableauIds) && d.linkedTableauIds.includes(id)
      )
    );
    if (linkedBy.length) {
      ...
      return res
        .status(400)
        .json({ error: `Impossible de supprimer : ce tableau est lié par ${linkedInfo}.` });
    }
...
router.put('/disjoncteur/:tableauId/:disjoncteurId', async (req, res) => {
  ...
  try {
    ...
    const result = await client.query('SELECT disjoncteurs FROM tableaux WHERE id = $1', [
      tableauId
    ]);
    if (!result.rows.length) return res.status(404).json({ error: 'Tableau non trouvé' });
    const disjoncteurs = Array.isArray(result.rows[0].disjoncteurs)
      ? result.rows[0].disjoncteurs
      : [];
    ...
    const updated = {
      ...
      icn: require('../utils/electric').normalizeIcn(
        updatedData.icn || disjoncteurs[idx].icn
      ),
      section:
        updatedData.section ||
        disjoncteurs[idx].section ||
        `${getRecommendedSection(updatedData.in || disjoncteurs[idx].in)} mm²`,
      cableLength: isNaN(parseFloat(updatedData.cableLength))
        ? disjoncteurs[idx].isPrincipal
          ? 0
          : 20
      ...
      linkedTableauIds: Array.isArray(updatedData.linkedTableauIds)
        ? updatedData.linkedTableauIds
        : [],
    ...
    await client.query('UPDATE tableaux SET disjoncteurs = $1::jsonb WHERE id = $2', [
      JSON.stringify(disjoncteurs),
      tableauId
    ]);
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la mise à jour du disjoncteur: ' + e.message });
  } finally {
    if (client) client.release();
  }
});
...
router.put('/equipement/:tableauId/:equipmentId', async (req, res) => {
  ...
  try {
    client = await pool.connect();
    const errors = require('../utils/validation').validateEquipementData({
      ...updatedData,
      id: newId
    });
    if (errors.length)
      return res.status(400).json({ error: 'Données invalides: ' + errors.join('; ') });
    const result = await client.query(
      'SELECT equipment_id, equipment_type, data FROM equipements WHERE tableau_id = $1 AND equipment_id = $2',
      [tableauId, equipmentId]
    );
    ...
    await client.query(
      ...
      [
        newId || decodeURIComponent(equipmentId),
        updatedData.equipmentType,
        JSON.stringify(data),
        tableauId,
        decodeURIComponent(equipmentId)
      ]
    );
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la mise à jour de l'équipement: " + e.message });
  } finally {
    if (client) client.release();
  }
});
...
router.delete('/equipement/:tableauId/:equipmentId', async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la suppression de l'équipement: " + e.message });
  } finally {
    if (client) client.release();
  }
});
      ...
      const disjoncteurs = (Array.isArray(row.disjoncteurs) ? row.disjoncteurs : []).map(d => {
        ...
        if (d.ue && (d.impedance || d.section)) {
          ...
          if (d.impedance) {
            ...
            const S = sectionMatch
              ? parseFloat(sectionMatch[0])
              : require('../utils/electric').getRecommendedSection(d.in);
        ...
        return {
          ...d,
          ik,
          icn: require('../utils/electric').normalizeIcn(d.icn),
          tableauId: row.id,
          linkedTableauIds: Array.isArray(d.linkedTableauIds)
            ? d.linkedTableauIds
            : d.linkedTableauId
...
router.post('/fault-level/update', async (req, res) => {
  ...
  try {
    ...
    const updatedData = {
      ue: ue || disjoncteurs[idx].ue,
      section: section || disjoncteurs[idx].section,
      cableLength: cableLength || disjoncteurs[idx].cableLength,
      impedance: impedance || disjoncteurs[idx].impedance
    };
