...
const { requireAuthBasic, requireAuth } = require('../middleware/authz');

// GET /accounts/mine → {accounts:[{id,name,role}]}
// -> accessible même si l'utilisateur n'a encore aucune membership
router.get('/accounts/mine', requireAuthBasic, async (req, res) => {
  try {
...
// -> doit être possible même sans membership existante
router.post('/accounts', requireAuthBasic, async (req, res) => {
...
router.delete('/accounts/:accountId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (e.code !== '42P01') throw e; // ignore si la table n'existe pas
    }
